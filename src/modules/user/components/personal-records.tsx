...
const getMostWordsInGame = (games: DbGame[]): number => {
	return games.reduce(
		(max, g) => Math.max(max, g.wordsUsed?.length ?? 0),
		0
	);
};
...
const getAverageWPG = (games: DbGame[]): number => {
	...
	const total = games.reduce(
		(sum, g) => sum + (g.wordsUsed?.length ?? 0),
		0
	);
...
const PersonalRecords = async ({ userId }: { userId: string }) => {
	const games = (await GetRecentGamesForUser(userId, Number.MAX_SAFE_INTEGER)) as DbGame[];

	// TODO - prepinani jazyku a podle toho menit
	const longestWord = getLongestWord(games.filter(g => g.language === 'en'));
	const mostWordsInGame = getMostWordsInGame(games.filter(g => g.language === 'en'));
	const averageWordsPerGame = getAverageWPG(games.filter(g => g.language === 'en'));
	const averageScorePerGame = getAverageScorePerGame(games.filter(g => g.language === 'en'));
	...
	return (
		<div>
			...
			<div className="grid gap-4 md:grid-cols-2">
				...
				<Card>
					<p className="text-sm text-surface-500">Average Words per Game</p>
					<p className="text-lg font-semibold">{averageWordsPerGame}</p>
				</Card>
				<Card>
					<p className="text-sm text-surface-500">Average Score per Game</p>
					<p className="text-lg font-semibold">{averageScorePerGame}</p>
				</Card>
			</div>
		</div>
	)
